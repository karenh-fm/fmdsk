...
int fmd_memory_alloc_cma_dsk(struct fmd_device_t *fmd, int nr_pages)
{
        struct page *page;
	...
	printk(KERN_INFO "%s: %s\n", fmd->dev_name, __func__);
        ...
        page = dma_alloc_from_contiguous((struct device *)disk_to_dev(fmd->disk), fmd->nr_pages, 0);
#else
        page = dma_alloc_from_contiguous((struct device *)disk_to_dev(fmd->disk), fmd->nr_pages, 0, GFP_KERNEL);
#endif
	if (!page) {
		printk(KERN_INFO "%s: %s dma_alloc_from_contiguous FAILED\n", fmd->dev_name, __func__);
		goto err_mem_alloc_cma_dsk;
	}
	fmd->virt = page_address(page);
	fmd->phys = page_to_pfn(page);

        return 0;

err_mem_alloc_cma_dsk:
        fmd_memory_cleanup_cma(fmd);
        return -ENOMEM;
}

int fmd_memory_alloc_cma_cache(struct fmd_device_t *fmd, int nr_pages)
{
        struct page *page;
	struct fmd_cache_t *cache;
	...
	cache = (struct fmd_cache_t *) fmd->cache;
        cache->nr_pages = nr_pages;
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,11,0)
        page = dma_alloc_from_contiguous((struct device *)disk_to_dev(fmd->disk), nr_pages, 0);
#else
        page = dma_alloc_from_contiguous((struct device *)disk_to_dev(fmd->disk), nr_pages, 0, GFP_KERNEL);
#endif
	if (!page) {
		printk(KERN_INFO "%s: %s dma_alloc_from_contiguous FAILED\n", fmd->dev_name, __func__);
		goto err_mem_alloc_cma_dsk;
	}
	cache->virt = page_address(page);
	cache->phys = page_to_pfn(page);
...
void fmd_memory_cleanup_cma(struct fmd_device_t *fmd)
{
	...
	if (fmd->phys) {
	    dma_release_from_contiguous((struct device *)disk_to_dev(fmd->disk), 
					pfn_to_page(fmd->phys), fmd->nr_pages);
	}
